...
def updateCanceled(time):
    conn = pymysql.connect(host=DBHOST, user=DBUSER, password=DBPWD, db=DB, charset='utf8')
    cur = conn.cursor()
    sql = '''update Reservation set status = "canceled" where exists((select id from Seat where Seat.time = %s and seatId=Reservation.seatId)) and DATE(Reservation.created_at)=DATE(now()) and Reservation.status = "prebooked";'''
    cur.execute(sql, time)
    conn.commit()
    conn.close()
...
def selectCanceledUser(time):
    conn = pymysql.connect(host=DBHOST, user=DBUSER, password=DBPWD, db=DB, charset='utf8')
    cur = conn.cursor()
    sql = '''select userId from Reservation where exists((select id from Seat where Seat.time = %s and seatId=Reservation.seatId)) and DATE(Reservation.created_at)=DATE(now()) and Reservation.status = "canceled";'''
    cur.execute(sql, time)
    res = cur.fetchall()
    conn.commit()
    conn.close()
